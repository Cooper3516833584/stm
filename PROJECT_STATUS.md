...
- `road_obstacle_flight_test.py` 是与单寻路入口隔离的固定视觉真实飞行测试。只有同时提供
  `--enable-flight --auto-takeoff --confirm-obstacle-flight-test` 才能运行；强制雷达绕障、
  保留记录，并限制速度为 10/8cm/s、偏航 10°/s、起飞高度不超过 100cm
...
- 安全 dry-run：`--no-fc --no-radar --dry-run --no-record` 使用 `fast-main` 连续推理
  109 帧、0 错误，推理线程约 13 FPS；全程 `fc_mode=no-fc`、`sent=False`，SIGINT
  后相机与推理线程正常停止
- 板端兼容修复：语义输入必须为 C-contiguous NCHW；旧 CPU YOLO 在无 `cv2.dnn`
  的 OpenSTLinux OpenCV 上自动使用纯 NumPy NMS
