...
`PASS_FORWARD` 并恢复 `13.2 cm/s` 绕障前速。`DIVERGE` 阶段横向速度按 95 cm 目标净空
计算；进入后续状态即恢复按 85 cm 目标净空计算。
该 `static-route` 融合入口完全旁路 `SafetyArbiter`：规划器输出不再经过前方停车、前方限速、
侧向停车、速度裁剪或飞行健康硬停车，直接转换为飞控实时控制帧。雷达/视觉预检、规划器自身的
路径丢失与避障状态机、飞行启动确认和退出降落流程仍保留；这些属于输入、任务和生命周期逻辑，
不是 Safety 对规划命令的二次仲裁。

障碍从 80°～90°侧边缘连续消失 3 帧后进入 `CLEARANCE_RUN`。该阶段不再关联或
传播已经离开的旧障碍，也不再用 `front_corridor_clear` 或预测后缘决定完成；飞机以
`13.2 cm/s` 持续前飞，最终实际执行的正向控制帧累计 1.5 秒且没有按首次捕获规则确认新的路径
障碍后，进入 `WAIT_VISUAL → BLEND_BACK`。命令未执行或发现新障碍都会中断该连续
时间，其中确认的新障碍作为新的 encounter 重新选边绕行。冻结 v1 的旧后缘预测逻辑仍保留用于
回归对照。

## 紫色目标与投放任务

默认 static-route 入口同时启动 10 Hz 紫色连通区域检测。目标连续确认后，目标状态机暂时替代巡线
期望命令，并经过同一个绕障规划器后直接发送；道路 NPU 全程保持运行。完成投放或放弃后只停止
目标检测，并立即恢复巡线。投放门不再读取 Safety 状态，完整状态和参数依据见
...
| 雷达、视觉、道路或规划状态异常 | 红灯亮 0.2 秒、灭 0.2 秒循环 |
