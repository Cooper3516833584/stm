...
4. static-route 下直接发送规划器命令；旧规划器模式仍运行 `SafetyArbiter`；
5. 在真实飞行模式下发送最终命令；
6. 更新状态指示灯；
7. 将小型元数据任务送入记录进程。

主循环不等待摄像头采集、NPU 推理、雷达串口读取、视频编码或点云压缩。static-route 下传感器
快照状态只进入视觉/规划器/任务逻辑，不再经过 Safety 硬停车；旧规划器模式保留原 Safety 行为。
...
融合运行时使用双雷达中较老的一路作为聚合年龄。任一路超过 0.5 秒、工作进程死亡、点云
共享内存读取失败或 CRC 健康门未恢复，均会令 `radar_fresh=False`。static-route 不再因此触发
Safety 停车；该状态仍用于记录、规划输入健康诊断和目标任务净空确认。
...
这只修复“瞬时 CRC 后永久不健康”的软件锁死。真实串口再次停帧或新增 CRC 时会重新标记雷达
不健康，但 static-route 的直通命令不会被 Safety 清零。
...
当前默认 static-route 的控制顺序为：

```text
视觉期望命令 → 绕障规划命令 → 飞控最终命令
```

static-route 不构造或调用 `SafetyArbiter`。80 cm 前向停车、150 cm 前向限速、45 cm 侧向停车、
命令速度裁剪以及飞控连接、模式、解锁、姿态、非法数值和电池门均不再改写规划器输出。
完整点云的前向距离与左右净空继续写入记录，仅用于复盘。日志将 `safety_state` 标为 `BYPASSED`、
`safe` 标为 `null`，并保持 `planned == final`。legacy、smooth-sidestep 和 circular 旧入口仍保留原
Safety 链路。
...
| 正常避障 | 红灯常亮 |
| 雷达/视觉失效、道路丢失、路径/跟踪丢失、failsafe、timeout | 红灯亮 0.2 秒、灭 0.2 秒循环 |
