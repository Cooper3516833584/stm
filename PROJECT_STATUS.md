...
**当前阶段**: M5 阶段 — 飞控联调完成 + 双雷达硬件接入中（下雷达已接 UART9，待软件验证）
**最后更新**: 2026年5月17日 (下雷达 UART9 接线完成，PROJECT_STATUS 雷达部分更新)
...
- [ ] 运行 `smoke_dual_radar.py` 验证下雷达连通性（`--front-port /dev/ttySTM4 --rear-port /dev/ttySTM9`）
- [ ] 确认下雷达支撑脚遮挡扇区，必要时调整安装方位角使遮挡避开前向走廊
- [ ] 配置 `MultiRadar` 的 `RadarConfig`：上雷达 `mount_yaw_deg=0`，下雷达 `mount_yaw_deg=0`（下倾角在 2D 坐标变换中不表达，后续路径规划中单独处理）
- [ ] 双雷达点云融合坐标系验证（`get_obstacle_points_body_cm()` 输出正确性）
