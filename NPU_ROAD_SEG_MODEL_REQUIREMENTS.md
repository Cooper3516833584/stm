...
The later `road_fastseg_256` test changed the project direction again:
...
road_fastseg_256 custom semantic model: .nb generation is now possible
```

The project should now stop cloud-side conversion experiments for
`road_fastseg_256` and move to board-side `.nb` contract, latency, NPU-driver,
and mask-quality validation.
...
| INT8 `.nb` | Generated by ST Cloud; must be copied into this repository/board path |
| `/dev/galcore` board proof | Pending board-side validation |
...
.nb: available from ST Cloud, pending board-side acceptance
```
...
2. Run `validate_nb_npu_contract.py` with raw STAI profiling.
3. Run `strace -yy` and confirm `/dev/galcore` open/ioctl.
4. Run `render_deeplab_nb_overlay.py` on saved road images and confirm the road
   class is visually plausible.
5. Only after those checks pass, integrate the semantic mask path into
   `road_perception.py`.
