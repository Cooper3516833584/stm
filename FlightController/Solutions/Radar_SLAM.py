...
def radar_find_target(
    ...
    # 定义你想要保留的区域的坐标 (x, y, width, height)
    x, y, w, h = x0, y0, x_out, y_out  # 雷达扫网内部区域

    # 在掩膜上绘制你想要保留的区域，这里我们使用白色（255）来表示
    mask[y-(h-20):y+(h-20), x-(w-20):x+(w-20)] = 255

    # 使用掩膜提取区域
    extracted_region = cv2.bitwise_and(image, image, mask=mask)
    contours , _ = cv2.findContours(extracted_region,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    points = []
    ...
    if debug:
        if debug_save_img:
            ...
            color = (0, 255, 0)  # 绿色
            for point in points:
                cv2.circle(extracted_region, tuple(point), 5, color, -1)

            cv2.imwrite("radar_getTarget_debug.png", extracted_region)

    return points , len(points)  # type: ignore