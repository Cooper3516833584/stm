...
class FastestDetOnnx(FastestDet):
    ...
    def __init__(self, confThreshold=0.6, nmsThreshold=0.2, drawOutput=False):
        ...
        logger.debug(f"onnxruntime available providers: {providers}")
        providers = ["CPUExecutionProvider"]  # force use CPU
        self.session = onnxruntime.InferenceSession(path_onnx, providers=providers)
        self.confThreshold = confThreshold
        self.nmsThreshold = nmsThreshold
        self.drawOutput = drawOutput
...
class DAMO_YOLO(object):
    def __init__(self, confThreshold=0.5, nmsThreshold=0.5, drawOutput=False):
        ...
        logger.debug(f"onnxruntime available providers: {providers}")
        providers = ["CPUExecutionProvider"]  # force use CPU
        self.session = ort.InferenceSession(path_onnx, so, providers=providers)
...
class HAWP(object):
    ...
    def __init__(self, confThreshold=0.95, drawOutput=False):
        ...
        logger.debug(f"onnxruntime available providers: {providers}")
        providers = ["CPUExecutionProvider"] # force use CPU
        self.onnx_session = onnxruntime.InferenceSession(path_onnx, providers=providers)
