...
`DIVERGE` 时保持绕障前速。该配置不再使用 75 cm 的
侧边缘提前出口，连续确认管表面净空达到原有 85 cm 后才恢复 `13.2 cm/s` 绕障前速。
Safety 的前方 80 cm 强制停车在该融合入口中关闭，
但前方限速、侧向保护和飞行健康硬停车仍保留。
...
| 即将起飞 | 红灯，等待 5 秒 |
| 正常巡线 | 绿灯 |
| 正常巡线受到 Safety 限速（`LIMITED`） | 绿灯 |
| 正常避障 | 红灯 |
| Safety 前方停车（当前融合配置已关闭） | 不适用 |
