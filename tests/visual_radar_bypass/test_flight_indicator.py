...
def test_indicator_uses_green_for_normal_red_for_avoidance_and_flashes_errors():
    fc = _FakeFC()
    indicator = FusionFlightIndicator(fc)

    indicator.update(now_s=0.0, avoiding=False, unexpected=False)
    indicator.update(now_s=0.1, avoiding=False, unexpected=False)
    indicator.update(now_s=0.2, avoiding=True, unexpected=False)
    indicator.update(now_s=0.41, avoiding=False, unexpected=True)
    indicator.update(now_s=0.61, avoiding=False, unexpected=True)
    indicator.update(now_s=0.81, avoiding=False, unexpected=False)
